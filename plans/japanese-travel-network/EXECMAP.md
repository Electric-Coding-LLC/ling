...
- Make `Japanese` and `Japan` compact orientation/reference stations. Make the four practical stations short playable reference lists with no review, score, progress, completion control, scheduling, speech evaluation, or persistence in this slice.
...
- `Japanese` briefly introduces Ling and the network; `Japan` briefly frames practical use in Japan; each practical station provides no more than six accurate, playable essentials in the shared restrained reference style.
