...
test("flashcards share directional touch gestures and answer transitions", async () => {
  ...
  assert.match(styles, /@media \(max-width: 600px\), \(hover: none\) and \(pointer: coarse\) \{[\s\S]*\.hiragana-test-answer-no\s*\{[^}]*color:\s*var\(--audio\)/s);
  ...
  assert.match(review, /export function KanaFlashcardContent[\s\S]*!revealed \? \([\s\S]*<FlashcardCountdown onComplete=\{onReveal\} \/>[\s\S]*\) : null/);
  ...
  for (const path of [...exampleGuidePaths, ...kanaOnlyGuidePaths]) {
    const guide = await readFile(new URL(path, root), "utf8");
    assert.match(guide, /from "\.\.\/flashcard-review"/);
  ...
  for (const path of exampleGuidePaths) {
    const guide = await readFile(new URL(path, root), "utf8");
  ...
  for (const path of kanaOnlyGuidePaths) {
    const guide = await readFile(new URL(path, root), "utf8");
    assert.match(guide, /<KanaFlashcardContent[\s\S]*onReveal=\{activateCard\}[\s\S]*pronunciationPlaying=\{activeAudioIndex === 0\}[\s\S]*revealed=\{pronunciationRevealed\}/);
    assert.doesNotMatch(guide, /getJapaneseWordRomaji|splitJapaneseMorae|function playExample|onPlayExample|Example:/);
  }
});
